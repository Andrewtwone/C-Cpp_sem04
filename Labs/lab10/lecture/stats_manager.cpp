...
void StatsManager::initializeStats(LearningStats& stats, const std::vector<Word>& words) {
...
void StatsManager::updateStats(LearningStats& stats, const std::string& word, bool correct) {
...
void StatsManager::displayCurrentStats(const LearningStats& stats) {
...
void StatsManager::saveUserStats(const UserStats& stats) {
    std::ofstream file(USER_STATS_FILE, std::ios::app);
    if (!file.is_open()) {
...
void StatsManager::loadUserStats(std::vector<UserStats>& stats) {
    std::ifstream file(USER_STATS_FILE);
    if (!file.is_open()) {
        return;
    }
...
void StatsManager::displayUserStats(const std::vector<UserStats>& stats) {
