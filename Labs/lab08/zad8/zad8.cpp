...
void copyWithProgress(int in_fd, int out_fd, const string &mode)
{
    ...
    while ((bytesRead = read(in_fd, buffer, BUF_SIZE)) > 0)
    {
        ...
        if (mode == "-p")
        {
            int percent = (int)((double)totalRead / totalSize * 100);
            cout << "\rSkopiowano: " << percent << "%" << flush;
        }
        else if (mode == "-b")
        {
            cout << "\rSkopiowano: " << totalRead << " / " << totalSize << " bajtow" << flush;
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    copyWithProgress(in_fd, out_fd, mode);
